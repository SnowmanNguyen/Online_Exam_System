...
include '../config.php';
error_reporting(0);
include __DIR__ . '/message_unread.inc.php';
$exid=$_POST['exid'];

if (!function_exists('student_parse_exam_datetime')) {
    function student_parse_exam_datetime($raw)
    {
        if ($raw === null || $raw === '') {
            return false;
        }
...
if (isset($_POST["edit_btn"])) {
  ...
  $subt = $row['subt'];
  $tEx = student_parse_exam_datetime($row['extime']);
  $tSub = student_parse_exam_datetime($row['subt']);
  if ($tEx === false && $tSub === false) {
    header("Location: exams.php");
    exit;
  }
  if ($tEx === false) {
  ...
  $windowStart = min($tEx, $tSub);
  $windowEnd = max($tEx, $tSub);
  $nowTs = time();
  if ($nowTs < $windowStart || $nowTs > $windowEnd) {
    header("Location: exams.php");
    exit;
  }
  $deadlineTs = max($tEx, $tSub);
  $td_iso = date('c', $deadlineTs);
    ...
    <link href='https://unpkg.com/boxicons@2.0.7/css/boxicons.min.css' rel='stylesheet'>
     ...
     <?php
         ...
         <script type="text/javascript">
          ...
          var count_id = "<?php echo htmlspecialchars($td, ENT_QUOTES, 'UTF-8'); ?>"; 
  ...
  <div class="sidebar active">
    <div class="logo-details">
      ...
      <ul class="nav-links">
        ...
        <li>
          <a href="messages.php" class="<?php echo !empty($studentMessageUnreadCount) ? 'nav-msg-with-badge' : ''; ?>">
            <i class='bx bx-message' ></i>
            <span class="links_name">Messages</span>
            <?php echo student_message_badge_html(); ?>
          </a>
        </li>
        <li>
          <a href="settings.php">
